defmodule BlendendPlayground.Swatches do
  ...
  defp do_render(%BlendendPlayground.Palette.Scheme{} = scheme) do
    ...
    draw width, height do
      ...
      {:ok, monospace_face} =
        Blendend.Text.Face.load(priv_font_path("MonaspaceArgon-Regular.otf"))
      ...
      text(serif_regular, width * 0.5, height * 0.05, "#{width} x #{height}", fill: stroke_color)
      ...
      Enum.with_index(scheme.colors, fn color, idx ->
        ...
        text(monospace_regular, x + 6, y + @box - 20, "rgb: #{r}, #{g}, #{b}", fill: stroke_color)
      end)
    end
  end
